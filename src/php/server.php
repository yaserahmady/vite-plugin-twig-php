...
$server = new HttpServer(static function (ServerRequestInterface $request) use (
    $twigRenderer,
    &$counter,
): Response|Promise {
    $headers = [
    ...
    if (!isset($query["type"])) {
        return new Response(
            ...
            json_encode([
                "ok" => true,
                "message" =>
                    "No action correctly requested. Url must have a query param of 'templatePath' for which twig template to render, but yes - the server is running.",
            ]),
        );
    }
    ...
    switch ($type) {
        ...
        case "renderFile":
            return new Promise(static function ($resolve, $reject) use (
                $twigRenderer,
                $body,
                $headers,
            ): void {
                $results = $twigRenderer->render(
                    $body["template"],
                    $body["data"],
                );
        ...
        case "renderString":
            return new Promise(static function ($resolve, $reject) use (
                ...
                $results = $twigRenderer->renderString(
                    $body["template"],
                    $body["data"],
                );
                $response = new Response(
...
// Only output server status if verbose
if ($config["verbose"]) {
    echo "PHP Twig Render Server listening on " .
